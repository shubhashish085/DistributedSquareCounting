...
void CountingAlgorithm::count_square_in_partitioned_graph_single_node(const std::string &file_path, const std::string &vertex_partition_file_path)
{
    ...
    VertexID wedge_begin, wedge_end;
    long long total_sq_count = 0, total_exact_count = 0;

    for (int partition_idx = 0; partition_idx < 8; partition_idx++)
    {

        Graph *graph = new Graph();
        NodeID ptn_idx, ptn_idx_1, ptn_root;
        ...
        for (ui i = 0; i < (graph->vertices).size(); i++)
        {
            ...
            for (ui j = 0; j < nbr_count; j++)
            {
                ...
                {
                    search_pair = get_wedge_endpoint_pair(neighbors[j], neighbors[k]);
                    ptn_root = graph->partition[graph->vertices[i]];
                    ptn_idx = graph->partition[neighbors[j]];
                    ptn_idx_1 = graph->partition[neighbors[k]];

                    if ((ptn_idx == ptn_idx_1) && (ptn_idx == partition_idx))
                    {
                        auto search_result = (graph->wedge_map).find(search_pair);
                        if (search_result == (graph->wedge_map).end())
                        {
                    ...
                    else if ((ptn_idx == partition_idx) || (ptn_idx_1 == partition_idx))
                    {
                        auto search_result = (wedge_map_comm).find(search_pair);
                        if (search_result == (wedge_map_comm).end())
                        {
                            wedge_map_comm[search_pair] = 1;
                        }
                        else
                        {
                            wedge_map_comm[search_pair] = wedge_map_comm[search_pair] + 1;
                        }
                    } else if ((ptn_idx != ptn_idx_1) && (ptn_root != ptn_idx) && (ptn_root != ptn_idx_1)) {
        ...
        std::cout << "Rank - " << partition_idx << " : wedge map count - " << (graph->wedge_map).size() << std::endl;
        std::cout << "Rank - " << partition_idx << " : wedge map communication count - " << (wedge_map_comm).size() << std::endl;
        ...
        std::cout << "Rank - " << partition_idx << " : Local Square Count - " << local_sq_count << std::endl;
    ...
    std::cout << "Total Exact Square Count : " << total_exact_count / 2 << std::endl;
}
...
void CountingAlgorithm::dist_opt_count_square_in_partitioned_graph(const std::string &file_path, const std::string &vertex_partition_file_path)
{
    ...
    if (world_rank == 0)
    {
        ...
        for (int rank = 1; rank < world_size; ++rank)
        {
            ...
            //std::cout << "Rank : " << world_rank << " - Received From : " << rank << " Buffer Size : " << buffer_size << std::endl;

            std::vector<VertexID> recv_buffer(buffer_size);
            MPI_Recv(recv_buffer.data(), buffer_size, MPI_INT, rank, TAG_BUFFER, MPI_COMM_WORLD, &buffer_status);
            ...
            while (i < recv_buffer.size())
            {
                ...
                search_pair = std::make_pair(recv_buffer[i], recv_buffer[i + 1]);
               

                if ((wedge_map).find(search_pair) != wedge_map.end())
                {
                    wedge_map[search_pair] = wedge_map[search_pair] + recv_buffer[i + 2];
                }
                else
                {
                    wedge_map[search_pair] = recv_buffer[i + 2];
                }
        ...
        std::cout << "Rank : " << world_rank << " : ====================Finished=======================" << std::endl;
    }
    else
    {
        Graph *graph = new Graph();
        ...
        for (ui i = 0; i < (graph->vertices).size(); i++)
        {
            ...
            for (ui j = 0; j < nbr_count; j++)
            {
                for (ui k = j + 1; k < nbr_count; k++)
                {
                    ...
                    if ((ptn_idx == ptn_idx_1) && (ptn_idx == (world_rank - 1)))
                    {
                        auto search_result = (graph->wedge_map).find(search_pair);
                        if (search_result == (graph->wedge_map).end()){
                            graph->wedge_map[search_pair] = 1;
                    ...
                    } else if ((ptn_idx == (world_rank - 1)) || (ptn_idx_1 == (world_rank - 1))) {
        ...
        //std::cout << "Rank " << world_rank << " Local Square Count : " << local_sq_count << std::endl;
        ...
        //std::cout << "Rank : " << world_rank << " Buffer - " << buffer[0] << " Buffer - " << buffer[1] << std::endl;

        MPI_Send(&buffer_size, 1, MPI_INT, MPI_MASTER, TAG_BUFFER_SIZE, MPI_COMM_WORLD);

        //std::cout << "Rank " << world_rank << " Buffer Size : " << buffer_size << " Sent." << std::endl;
      

        MPI_Send(buffer.data(), buffer_size, MPI_INT, MPI_MASTER, TAG_BUFFER, MPI_COMM_WORLD);

        //std::cout << "Rank " << world_rank << " buffer data sent." << std::endl;
...
long long CountingAlgorithm::opt_communication_cost_analysis(const std::string &vertex_partition_file_path, const std::string &file_path, int partition_no)
{
